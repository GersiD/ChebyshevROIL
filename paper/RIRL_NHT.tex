...
We assume the $\gamma$-discounted infinite horizon objective with $\gamma \in [0,1)$. The infinite-horizon discounted return of a policy $\pi \in \Pi$ and a reward $r \in \mathcal{R}$ is denoted by $\rho(\pi, r)$. The set of deterministic policies is $\Pi_D= \mathcal{A}^{\mathcal{S}}$ and the set of randomized policies is $\Pi_{\mathrm{R}} = {\left(\Delta^{\mathcal{A}}\right)}^{\mathcal{S}}$. The set of all policies is $\Pi = \Pi_{\mathrm{R}} \cup \Pi_D$.
...
\begin{equation}\label{eq:policy-construction}
	\pi(s, a) \; =\;  \nicefrac{u(s,a)}{\sum_{a' \in \mathcal{A}} u(s,a')}, \qquad \forall s\in \mathcal{S}, a\in \mathcal{A}.
\end{equation}
...
Modern IRL algorithms, like GAIL, usually add other regularization or penalty to the scheme in \eqref{eq:irl-formulation-u} and use $\phi$-divergence based set $\mathcal{W}$. In this work, we do not consider these extension in order to focus on the fundamental properties and tradeoffs of this formulation.

An important limitation of the formulation in \eqref{eq:irl-formulation-u} is that it relies on estimating the expert's occupancy frequency $\hat{u}_{\mathrm{e}}$ well. The occupancy frequency represents the frequency of \emph{both} states and actions is, therefore, very sensitive to the initial distribution and covariate shifts in state distributions which may often arise in imitation learning settings. As discussed in the introduction, the expert may focus on difficult state when performing the demonstrations. In the remainder of the paper, we build on \eqref{eq:irl-formulation-u} to address its sensitivity to the initial distribution. 
...
\end{equation}
Here, $ \Pi_{\mathrm{R}}(\mathcal{D})$ represents the set of all policies that are consistent with the observations in $\mathcal{D}$ and are defined as 
%
...
\end{equation}
\mm{is there a reason we are using randomized policies here?}
...
\begin{equation}\label{eq:consistent-occupancies}
  ...
  c(s,a) =
  \begin{cases}
    1 &\text{if} (s,a) \notin \mathcal{D} \wedge \exists a'\in \mathcal{A}, (s, a') \in \mathcal{D} \\
    0 & \text{otherwise}.
  \end{cases}
\end{equation}
...
\label{lemma:occ_freq_matching}
    	Assume that $\mathcal{D}$ is the set of trajectories generated by some unknown deterministic policy $\pi_{\mathrm{e}} \in \Pi_D$. Then
\[
  u \in \Upsilon(\mathcal{D})
  \quad \Leftrightarrow \quad
  \exists \pi \in \Pi_{\mathrm{R}}(\mathcal{D}), \, u = u^{\pi}~,
\]
for each $\pi \in \Pi$ where $u^{\pi}$ is the occupancy frequency of the policy $\pi$.
\end{restatable}

Observe that given \cref{lemma:occ_freq_matching} maximizing over the policy space is equivalent to maximizing over the occupancy frequency space.
...
\subsection{Incorporating Additional Constraints}
...
\label{linf_constraint}
 ...
 \| (u - \hat{u_{\mathrm{e}}})\tr \Phi \| \le \eps 
 \right\}~.
\end{equation}
% \begin{equation}
% 	\Upsilon(\mathcal{D}) = \left\{ u \in \mathcal{U} \mid  
...
%  
\subsection{Geometric Intuition: Reduction to Chebyshev Center}
...
\begin{restatable}[Expert Recovery]{prop}{expertRecovery}
    ...
    and $u \in \Upsilon(\mathcal{D}) \Rightarrow u = u^{\pi_{\mathrm{e}}}$.
\end{restatable}
%
\begin{proof}
	Given $\mathcal{D}_\mathcal{S} = \mathcal{S}$ then $|\Pi_{\mathrm{R}}(\mathcal{D})| = 1$
	because there can only be one policy consistent with the demonstration
	$\mathcal{D}$ since every state was mapped to an action by $\pi_{\mathrm{e}} \in \Pi_D$
	which generated $\mathcal{D}$. Therefore, by Lemma 1, $u^\pi$ for $\pi \in
		\Pi_{\mathrm{R}}(\mathcal{D})$ is the only occupancy frequency consistent with the
	demonstration, and thus $|\Upsilon(\mathcal{D})| = 1$. By the above
	statements, $u \in \Upsilon(\mathcal{D}) \Rightarrow u = u^{\pi_{\mathrm{e}}}$.
\end{proof}
...
\end{tikzpicture}
  \caption{An Illustration of MDP used in \cref{exm:roil-limitations}.}
  \label{fig:roil-fail}
\end{figure}
...
\] 
Now we show that minimizing either the GAIL or LPAL objective given the above dataset does not converge to $u^*_{\mathrm{e}}$. While by Proposition~\ref{expert_recovery} ROIL is guaranteed to recover the expert.
...
\begin{proof}
	...
	Case 2: Prove $\pi \in \Pi_{\mathrm{R}}(\mathcal{D}) \quad \Rightarrow \quad  u^\pi \in
		\Upsilon (\mathcal{D})$
