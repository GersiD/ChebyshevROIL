...
\begin{abstract}
We study the problem of imitation learning via inverse reinforcement learning where the agent attempts to learn an expert's policy from a dataset of collected state, action tuples. We derive a new robust model-based offline Imitation Learning method (ROIL) that mitigates covariate shift by avoiding estimating the expert's occupancy frequency. Frequently in offline settings, there is insufficient data to reliably estimate the occupancy frequency and this leads to models that do not generalize well. Expert demonstrators may also be biased, or worse, adversarial \db{what does adversarial mean? wouldn't we want to avoid imitation learning in this setting?}. Our proposed approach, ROIL, a method that is guaranteed to recover the occupancy frequency \db{whose occupancy frequency, is it guaranteed to always recover, it if did wouldn't we always have zero regret?} and is efficiently solvable as an LP. We demonstrate ROIL's ability to perform well in a large grid world environment even under large covariate shift such as when the state visitation frequency of the demonstrator comes from a completely random policy. \db{this last sentence may be a bit confusing since it almost sounds like random demonstrations. Can we say where the demonstrations come from disjoint state-action pairs chosen randomly? Or is that not the same thing as what you mean?}
\end{abstract}
...
We assume that the domain can be modeled as a \emph{Markov Decision Process} with a finite number of states $\mathcal{S} = \left\{ 1, \dots , S \right\}$ and a finite number of actions $\mathcal{A} = \left\{ 1, \dots , A \right\}$. The transition probability function $p\colon \mathcal{S} \times \mathcal{A} \to \Delta^{\mathcal{S}}$, where $\Delta^{\mathcal{S}} = \left\{ x\in \Real_{+}^S \mid \sum_{s\in \mathcal{S}} x_s =1 \right\}$ is the probability simplex over the elements of the set $\mathcal{S}$. The reward function $r\opt \colon \mathcal{S} \times \mathcal{A} \to \Real$ represents the reward obtained in each transition. The initial distribution over states is denoted by $p_0\in \Delta^S$ and we assume that $p_0 > 0$.
...
\end{equation}
This construct is possible because $p_0 > 0 $ implies that the denominator in the construction above is always positive.
